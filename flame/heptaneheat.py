...
workbook = xlsxwriter.Workbook('C:\\Users\\shank\\Desktop\\PS 6 Flame\\heptaneheat.xlsx')
...
row += 1
cap = cv2.VideoCapture("C:\\Users\\shank\\Desktop\\PS 6 Flame\\heptane.mp4");
while(True):
    # Capture two frames
    ret, frame1 = cap.read()  # first image
    time.sleep(1/17)          # slight delay
    ...
    ret,thresh = cv2.threshold(blur,210,255,cv2.THRESH_OTSU)
    ...
    img2 = cv2.rectangle(img2,(200,100),(420,270),(0,255,0),2)
    (h,w)=img2.shape[:2]
    center=(w/3,h/3)
    angle= -90
    scale= 1

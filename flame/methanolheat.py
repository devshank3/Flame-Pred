...
workbook = xlsxwriter.Workbook('C:\\Users\\shank\\Desktop\\PS 6 Flame\\methanolheat.xlsx')
...
row += 1
cap = cv2.VideoCapture("C:\\Users\\shank\\Desktop\\PS 6 Flame\\Methanol-1.mp4");
while(True):
    # Capture two frames
    ...
    ret,thresh = cv2.threshold(blur,140,255,cv2.THRESH_OTSU)
    
    # combine frame and the image difference
    img2 = cv2.addWeighted(frame1,0.1,img1,0.9,0)
    ...
    img2 = cv2.rectangle(img2,(190,110),(390,260),(0,255,0),2)
    (h,w)=img2.shape[:2]
    center=(w/3,h/3)
    angle= -90
    scale= 1
